...
def parse_args():
    ...
    p.add_argument("--task1", type=Path, default=Path("MC1_24365.pth"))
    p.add_argument("--task2", type=Path, default=Path("Mc2_24365.pth"))
    p.add_argument("--task3", type=Path, default=Path("MC3_24365.pth"))
...
def main():
    ...
    for col,ylabel,suf in [("entropy_bits","Shannon marginal entropy (bits)","entropy_bits"),
        ...
        plt.figure(figsize=(9,5)); x=np.arange(len(layers)); width=.8/3
        for k,t in enumerate(TASKS):
            z=E[E.task==t].set_index("layer").reindex(layers)
            plt.bar(x+(k-1)*width,z[col].to_numpy(),width=width,label=t)
        plt.xticks(x,layers,rotation=25,ha="right"); plt.ylabel(ylabel); plt.title(f"{ylabel} by task and layer"); plt.legend()
        savefig(plots/suf)
